...
- `web/` — marketing/distribution website, Astro static site (see [Website](#website) below)
...
**Live at [questbot-web.vercel.app](https://questbot-web.vercel.app)** (Vercel's free subdomain — no custom domain yet). Astro static site in `web/`, a separate npm workspace from `server`/`client`. First phase of what will eventually be a real marketing/distribution site — see [docs/superpowers/specs/2026-07-31-marketing-website-shell-design.md](docs/superpowers/specs/2026-07-31-marketing-website-shell-design.md) for the full design.

- **Structure:** header with a game-tab row (Elden Ring active; other slots reserved but disabled, "coming soon") and a Download button, fixed across every page. Selecting Elden Ring reveals two real, separate static routes — `/elden-ring/wiki` (a "Coming soon" placeholder — the real wiki, turning the ~2166-item content corpus into browsable pages, is its own future phase) and `/elden-ring/map` (a real, fully working port of the interactive map).
- **The map port turned out to be small.** `client/src/renderer/map/{regions,locations,render,geometry,roles}.ts` had zero Electron/IPC dependencies, and the one cross-file dependency (favorites) already used plain `localStorage` — both verified by direct inspection before committing to the port, not assumed. Copied into `web/src/map/` with one real adaptation: the shape-editor dev tooling (`initEditMode`, `renderEditOutput`, and their four DOM lookups) was dropped, since those elements don't exist on the public site and used non-null assertions that would have crashed the module on load — a genuinely useful, unrelated safety-net function (clearing stray pin-hover clones on window blur) was kept, split into its own `initHoverSafetyNet()`.
- **A real routing lesson, found live:** the original design used a `?tab=map` query parameter for sub-tab switching. That doesn't work with Astro's static output — static HTML is pre-rendered once at build time, so a query string has nothing server-side to act on, even in `astro dev`. Fixed by using separate real routes instead (`/elden-ring/wiki`, `/elden-ring/map`), which is also more idiomatic for a static site and better for bookmarking/sharing.
- **Verified live in a real deployed browser** (not just `astro dev`): SVG renders correctly (11 region paths, correct viewBox), search-to-center works, the Surface/Underground layer toggle works, the Locations/Bosses filters work, the favorites-star toggle persists across a real page reload via `localStorage`, and the pin-hover clone mechanism works. The deployed Download button was confirmed to use the correct absolute URL, not a broken relative path.
- **The installer, packaged for the first time.** `client/package.json`'s `electron-builder` script had never actually been run in this project — it failed on the first attempt (`electron-builder` requires an exact pinned Electron version, not the `^43.1.1` range that was there; fixed by pinning to the exact installed `43.1.1`). Once fixed, it produced a real ~199MB NSIS installer, confirmed **genuinely unsigned** (`Get-AuthenticodeSignature` → `NotSigned`, not just assumed), installed and smoke-tested successfully on a real machine (Start Menu shortcut created, proper uninstall registry entry, app launches). Published as [GitHub Release v0.1.0](https://github.com/sosatyler39-arch/questbot/releases/tag/v0.1.0) — this also required creating this repo's first-ever GitHub remote and pushing, since none existed before this. The website's Download button links directly to that release asset; a note next to it warns about the expected Windows SmartScreen "Unknown Publisher" prompt, since there's no code-signing certificate yet.
...
**Weapons (480 items, 42 category pages, `data/weapons.ts`) and Armor (704 items, 4 category pages, `data/armor.ts`).** Unlike every other category above, these needed exact numeric stat blocks (attack power/scaling/requirements per weapon; defense/resistances/weight per armor piece) at a scale — Elden Ring's full base+DLC roster — well beyond what could reasonably be entered by hand. Project policy still holds (never scraped, never from model recall): the numbers come from two community projects that extract this data directly from the game's own files — [`ThomasJClark/elden-ring-weapon-calculator`](https://github.com/ThomasJClark/elden-ring-weapon-calculator) (MIT) for weapons, [`jerpdoesgames/EldenRingArmorOptimizer`](https://github.com/jerpdoesgames/EldenRingArmorOptimizer) (GPL-3.0, data-mined by a third party credited as "TarnishedSpreadsheet") for armor. Both include Shadow of the Erdtree content — the weapon source flags DLC items explicitly (102 of 480 tagged); the armor source has no such flag, so DLC armor (107 of 704, classified from general game knowledge in the location file only — `armor.ts` itself carries no DLC tag or flag at all) is included but untagged in the corpus text. **Known gap**: the weapon source doesn't carry weight or innate Ash of War name (armor.ts does have weight) — not guessed at, since that would violate the never-scraped/never-from-model-recall policy; flagged in `weapons.ts`'s header as an open follow-up. Weapon/armor location data (`data/item-locations/{weapons,armor}-locations.ts`) was authored the same way as every other category's location file — general game knowledge, no scraping. Synced and retrieval-verified live: "What is the attack power and stat requirements on the Uchigatana?" → 0.805 confidence, correct 115 physical / Str 11 Dex 15 answer plus its NPC drop location; "What are the defensive stats on the Bull-Goat Helm?" → 0.808 confidence, correct numbers plus its Volcano Manor location.

**Corpus audit against ERDB — partially completed, most of it deferred.** [ERDB](https://github.com/EldenRingDatabase/erdb) (MIT, extracts data directly from the game's files) was meant to both source Weapons/Armor and double-check every number already authored in the other 8 item categories. It turned out to be too old (last updated July 2023) to parse the current, DLC-patched game version's `regulation.bin` at all — confirmed by actually running it against a real, correctly UXM-unpacked install (`Unknown DCX format` on decrypt), not just a version-number guess. Weapons/Armor were rescued by the two community sources above. The audit itself only got as far as **Incantations and Sorceries** (a working base-game data source was found: [`jerpdoesgames/EldenRingSpellComparer`](https://github.com/jerpdoesgames/EldenRingSpellComparer), GPL-3.0) — 213 entries checked, 26 real numeric corrections found and applied (e.g. Dragonclaw's Stamina cost was wrong at 50, corrected to the real 35), full results in [`docs/superpowers/audit-logs/2026-07-22-corpus-audit.md`](docs/superpowers/audit-logs/2026-07-22-corpus-audit.md). **Talismans, Consumables/Crystal Tears/Whetstones, and Ashes of War remain unaudited** — no working replacement data source was found for those categories; this is an open follow-up, not a silently-dropped requirement. Retrieval-verified live post-correction: "How much Stamina does the Dragonclaw incantation cost?" → 0.825 confidence, correctly answers 35 (not the old wrong 50).

### Item location index

Every item across the 9 categories above (Talismans, Incantations, Sorceries, Consumables, Ashes of War, Crystal Tears, Whetstones + Great Runes, plus Weapons and Armor — 1892 items) now has a "where do I find this" entry in a new `item_locations` table, separate from the `chunks`/pgvector pipeline entirely (no embedding involved). Full design in [docs/superpowers/specs/2026-07-21-item-location-index-design.md](docs/superpowers/specs/2026-07-21-item-location-index-design.md), implementation plan in [docs/superpowers/plans/2026-07-21-item-location-index.md](docs/superpowers/plans/2026-07-21-item-location-index.md).
...
- **Verified per category**: item counts checked 1:1 against each source file, every referenced map pin checked against the real `MapLocation.name` list in `client/src/renderer/map/locations.ts` (300 references across all 9 categories, zero invalid), synced live, and spot-checked against real `/ask` questions where the daily embed quota allowed (Radagon Icon → Volcano Manor at 0.72-0.74 confidence, Sacrificial Twig → Church of Elleh, Golden Braid → honest DLC note with no fabricated pin).
- **Known limitation, found during final verification, now 7 collisions**: `item_locations.item_name` is a primary key, and 7 items are genuinely named the same across two categories. Six because the game itself reuses a spell's name for its matching Ash of War (Glintstone Pebble, Glintblade Phalanx, Carian Greatsword, Thops's Barrier, Carian Retaliation, Golden Vow) — checked all 6 by hand, every pair has consistent or identical location text, so no answer is wrong there, just occasionally less specific. The 7th, found during the Weapons/Armor whole-branch review, is a real bug rather than a benign duplicate: "Beast Claw" names both a base-game Bestial-cult incantation and an unrelated Shadow of the Erdtree fist weapon — the weapon's row (wrongly claiming DLC/no location) silently overwrote the incantation's correct Weeping Peninsula entry, because it happened to sync later. Manually restored the correct incantation row. A future composite key (name + source category, derivable from the chunk's URL prefix) would resolve this class of bug properly, and a cheap same-session-buildable invariant test (every `*_LOCATIONS` `itemName` globally unique) would catch new collisions like this one before they ship — not done here to avoid unplanned scope expansion on top of an already-large branch.
- Final distinct row count: **1885** (1892 authored − 7 real name collisions).
