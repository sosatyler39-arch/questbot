...
**Ashes of War (92 items, 14 affinity category pages, `data/ashes-of-war.ts`)** — weapon-type compatibility and effect descriptions for every affinity group (Heavy, Keen, Quality, Magic, Fire, Flame Art, Lightning, Sacred, Poison, Blood, Cold, Occult, Standard, plus the Lost Ashes of War duplication item) supplied directly by the user. Wired into `ALL_PAGES` and typechecked (92/92 verified against source), but **not yet synced into the DB** — the free-tier daily embed quota (1000/day) was exhausted again by the cumulative embed calls from the same day's earlier syncs (sorceries retry, the 642-chunk full re-embed, consumables, crystal tears/whetstones/great runes) before a single Ashes of War chunk went through. Re-run a targeted sync of `ASH_OF_WAR_PAGES` once the quota resets.
