...
**Ashes of War (92 items, 14 affinity category pages, `data/ashes-of-war.ts`)** — weapon-type compatibility and effect descriptions for every affinity group (Heavy, Keen, Quality, Magic, Fire, Flame Art, Lightning, Sacred, Poison, Blood, Cold, Occult, Standard, plus the Lost Ashes of War duplication item) supplied directly by the user. Wired into `ALL_PAGES` and typechecked (92/92 verified against source), but **still not synced into the DB as of the item-location-index work below** — every retry has hit the free-tier daily embed quota (1000/day) before a single Ashes of War chunk went through (`select count(*) from chunks where url like 'questbot://guide/ash-of-war/%'` → 0). **Practical consequence:** until this resolves, `/ask` can't return a confident match for any Ash of War question, so the 92 Ash of War location entries added below are correctly authored and synced into `item_locations` but currently unreachable through `/ask` (the location lookup only fires after a confident `chunks` match). Re-run a targeted sync of `ASH_OF_WAR_PAGES` once the quota resets — no code changes needed, just re-run.
