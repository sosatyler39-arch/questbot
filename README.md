...
**Incantations (129 items, 14 category pages, `data/incantations.ts`)** were added the same way — original phrasing, exact FP cost (including the continuous/per-second drain on channeled spells), memory slot cost, INT/FAI/ARC requirements, Stamina cost, and damage/buff/healing/utility classification all supplied directly by the user. Wired into `ALL_PAGES` and typechecked, but **not yet synced into the DB** — see the quota gap below, which now blocks this content too.

**Known gap — corpus-wide re-embed in progress, blocked on quota.** While verifying talisman retrieval, found that `embedAndStore` (`sync.ts`) only ever embedded `chunk.content`, never `chunk.title` — so a short generic effect string like "Shortens spell casting time." never mentions the item's own name anywhere in what actually gets embedded, and a query like "what does the Radagon Icon do?" can't find its own chunk by name. Fixed to embed `title: content` together (stored/displayed content is unchanged). This is a corpus-wide fix, not talisman-specific, but re-embedding all 429 chunks hit Gemini's free-tier **daily** embed quota (1000/day — separate from the per-minute one `embeddings.ts` already retries around) partway through: **280/429 chunks have the fix, 149 are still on the old formula** until the daily quota resets. The quota was still exhausted as of the incantations addition — a targeted sync of just the 129 new incantation chunks failed immediately on the first `embedContent` call with the same `PerDay` error. Once the daily quota resets, re-run `syncGame('elden-ring')` once to pick up both the remaining 149 old-formula chunks and the entire unsynced incantations corpus in one pass.
